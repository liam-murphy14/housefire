...
class HousefireAPI:
    ...
    def __clean_endpoint(self, endpoint: str):
    ...
    def __get(self, endpoint, params=None) -> r.Response:
        return r.get(
            self.__clean_endpoint(endpoint), headers=self.headers, params=params
        )

    def __post(self, endpoint, data=None) -> r.Response:
        return r.post(self.__clean_endpoint(endpoint), headers=self.headers, json=data)

    def __delete(self, endpoint) -> r.Response:
        return r.delete(self.__clean_endpoint(endpoint), headers=self.headers)
